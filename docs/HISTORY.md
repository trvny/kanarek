...
- Bundlowany `tv.m3u8` (merge dwóch wklejonych list IPTV, dedup po URL; #20) obok
  istniejącego `radio.m3u8` — żaden z nich jeszcze nigdzie automatycznie nieładowany,
  jedyna droga do playera to ręczny „Import M3U” przez SAF
...
- Player: `tv.m3u8`/`radio.m3u8` wciąż nigdzie nie są ładowane automatycznie (pierwsze
  uruchomienie) — jedyna droga do playera to ręczny „Import M3U” przez SAF
